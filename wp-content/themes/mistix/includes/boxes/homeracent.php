...
if($data['racent_status'] == '1' and $data['racent_status_port'] == '1'){
	$pc = new WP_Query(array('orderby=date', 'showposts' => $data['home_recent_number'], 'nopaging' => 0, 'post_status' => 'publish', 'ignore_sticky_posts' => 1, 'post_type' => array( 'post', 'portfolioentry'))); 
}
else if($data['racent_status'] != '1' and $data['racent_status_port'] == '1' ) {
	...
	$pc = new WP_Query(array('orderby=date', 'showposts' => $data['home_recent_number'], 'nopaging' => 0, 'post_status' => 'publish', 'ignore_sticky_posts' => 1, 'post_type' => array( 'post'))); 
}
else{
	$pc = new WP_Query(array('orderby=date', 'showposts' => $data['home_recent_number'], 'nopaging' => 0, 'post_status' => 'publish', 'ignore_sticky_posts' => 1, 'post_type' => array( 'post', 'portfolioentry'))); 
}
...
<div class="homeRacent">
	<div id="homeRecent">
		...
		//category
		$entrycategory = get_the_term_list( $post->ID, $catType, '', ', ', '' );		
		//end category		
		...
		if($count != 5){
			echo '<div class="one_fourth itemhome '.$type .'" data-category="'.$type .'">';
		}
		else{
			echo '<div class="one_fourth itemhome '.$type .'" data-category="'.$type .'">';
			$count = 0;
